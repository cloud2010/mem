...
function get_banner_slide(){
    ...
    if(is_array($previous_posts)){
        foreach($previous_posts as $post){
            ...
            $str.="<li><img src='".$thumbnail_img."' alt='".$post->post_title."' /></li>"; 
        }
    }
    echo $str;
}
    ...
    if(is_array($previous_posts)){
        foreach($previous_posts as $post){
            ...
            if(mb_strlen($post->post_title) > 23) {
                $post_title = mb_strimwidth($post->post_title,0,37)."...";               
            
            }  else  if(mb_strlen($post->post_title) <= 10) {
                $post_title = mb_strimwidth($post->post_title,0,39)."<br>"."<br>";
            
            } else {        
                $post_title = mb_strimwidth($post->post_title,0,39);
            }
        
            $post_content = mb_strimwidth($post->post_content,0,120);
            // $str.="<li class='bt-30'> <img src='".get_bloginfo('template_url')."/static/img/tu-".$i.".png' alt='".$post_title."' 
            ...
            $str.="<li class='bt-30'> <img src='".get_bloginfo('template_url')."/static/img/tu-".$i.".png' alt='".$post_title."' 
                title='".$post_title."'/><a href='".get_permalink($post->ID)."'><h4 title='阅读更多'>".$post_title."</h4></a><a href='".get_permalink($post->ID)."'>— 点击查看内容 —</a> </li>";
            $i++;
        }
    }
    echo $str;
...
//获取main-services的list，无图片
function get_main_servives_list() {
    $previous_posts = get_posts('numberposts=4&meta_key=main_services_list&meta_value=on&order=asc');
    $str = "";
    if(is_array($previous_posts)){

        $str.="<dt><a href='category/news/notice/'><h2>通知公告</h2></a></dt>";

        foreach($previous_posts as $post){
            setup_postdata($post);
            $post_title = mb_strimwidth($post->post_title,0,16);
            $post_content = mb_strimwidth($post->post_content,0,120);
            $str.="<dd > <a href='".get_permalink($post->ID)."'><h4 title='阅读更多'>".$post_title."</h4></a><p>".$post_content."</p><a href='".get_permalink($post->ID)."'></a> </dd>";
        }
        
        $str.='<dd ><a id="more" href="category/news/notice/">更多通知公告 > </a></dd>';
